...
namespace TestProject.UnitTest.Domain
{
    public class MessageServiceTest
    {
        ...
        public async Task ReceiveMessageAsync_ShouldCallGateway()
        {
            ...
            _messagerGatewaySubstitute.ReceiveMessagesAsync().Returns(Task.FromResult<MessageModel?>(messageModel));

            // Act
            var result = await _messageService.ReceiveMessageAsync();

            // Assert
            await _messagerGatewaySubstitute.Received(1).ReceiveMessagesAsync();
